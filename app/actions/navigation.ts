...
export const navigateMessaging = (chatId, friendUsername, friendUid) =>
  nav('Messaging', { chatId, friendUsername, friendUid });
...
export const navigateFilePreview = (type, uri, message = false, text) =>
  nav('FilePreview', { type, uri, message, text });
export const navigateGym = id => nav('Gym', { id });
export const navigateWelcome = (goBack?: boolean) => nav('Welcome', { goBack });
...
export const navigateSessionInfo = (sessionId, isPrivate) => nav('SessionInfo', { sessionId, isPrivate });

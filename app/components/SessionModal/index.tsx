    ...
    if (friends[host]) {
      return (
        <TouchableOpacity onPress={() => viewProfile(host)}>
          <Text style={{ color: colors.secondary }}>{friends[host].username}</Text>
        </TouchableOpacity>
      );
    }
    if (users[host]) {
      ...
      {session && (
        <View style={{ flex: 1 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', flexWrap: 'wrap' }}>
            <Text style={{ fontSize: 20, textAlign: 'center', padding: 10, color: '#000' }}>{session.title}</Text>
            <TouchableOpacity onPress={() => viewSession(session.key, session.private)}>
          ...
          <ScrollView style={{ margin: 10 }}>
            ...
            <Text style={{ marginVertical: 5, color: '#000' }}>
              {`${formatDateTime(session.dateTime)} for ${session.duration} ${session.duration > 1 ? 'hours' : 'hour'}`}
            </Text>

            <View
              style={{ flexDirection: 'row', marginVertical: 5, alignItems: 'center', justifyContent: 'space-between' }}
            >
              <Text style={{ flex: 1 }}>
                ...
                <Text style={{ color: '#999' }}>
                  {` (${
                    session.distance
                      ...
                      : getDistance(session, location.lat, location.lon).toFixed(2)
                  } km away)`}
                </Text>
              </Text>
              <Button
          ...
          <View style={{ justifyContent: 'flex-end', flex: 1, margin: 10 }}>{fetchButtons(profile.uid)}</View>
        </View>
      )}
    </Modal>
  );
